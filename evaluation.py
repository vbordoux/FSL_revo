...
# TODO here - need to change the selection of the file to test to not select all the
# subfolders in the ground truth folder but only the one with the right type of call. But it can be useful for future tests.
UNK_VALUE = 'UNK'

def remove_shots_from_ref(ref_df, number_shots=5, call_type='POS'):
    ref_pos_indexes = select_events_with_value(ref_df, call_type=call_type)
...
def select_events_with_value(data_frame, call_type='POS'):

    indexes_list = data_frame.index[data_frame["Q"] == call_type].tolist()
...
def compute_tp_fp_fn(pred_events_df, ref_events_df, call_type):
    ...
    ref_pos_indexes = select_events_with_value(ref_events_df, call_type=call_type)

    if "Q" not in pred_events_df.columns:
        pred_events_df["Q"] = call_type

    #sort events by starttime
    pred_events_df = pred_events_df.sort_values(by='Starttime', axis=0, ascending=True)
    pred_pos_indexes = select_events_with_value(pred_events_df, call_type=call_type)
    ...
    ref_unk_indexes = select_events_with_value(ref_events_df, call_type=UNK_VALUE)
...
def evaluate(conf, pred_file_path, ref_file_path, team_name, dataset, savepath, metadata=[], write_report=False):

    pos_type = conf.features.call_type

    print(f"\nEvaluation for:{team_name} on dataset {dataset} on call {pos_type}")
    ...
    # Adaptation of the code bellow to only select one folder
    subset = list_of_subsets[0][:-1] + pos_type
    ...
    # To evaluate all the subfolders in GT, currently not working because call_type is different
    ...
    for audiofilename in list(pred_events_by_audiofile.keys()):
        ...
        ref_events_this_audiofile = remove_shots_from_ref(ref_events_this_audiofile_all, number_shots=conf.train.n_shot, call_type=pos_type)
        
        # compare and get counts: TP, FP .. 
        tp_count, fp_count, fn_count , total_n_events_in_audiofile = compute_tp_fp_fn(pred_events_by_audiofile[audiofilename], ref_events_this_audiofile, call_type=pos_type)
    ...
    for audiofilename in list_all_audiofiles:
        if audiofilename not in counts_per_audiofile.keys():
            ...
            total_n_pos_events_in_audiofile =  len(select_events_with_value(ref_events_this_audiofile, call_type=pos_type))
